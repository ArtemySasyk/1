...
namespace FactoryWinForm
{
    public partial class ReportForm : Form
    {
        ...
        {
            _connection = new NpgsqlConnection("Server=localhost; Port=5432; User Id=postgres; Password=postpass; Database=KutMag");
        }
        ...
        private void button_action_Click(object sender, EventArgs e)
        {
            ...
            if (n == 0)
                return;
            selectedNames += $"p.product = '{dataGridView_clients.SelectedRows[0].Cells[1].Value}'";
            for (int i = 1; i < n; i++)
            {
                selectedNames += $" OR p.product = '{dataGridView_clients.SelectedRows[i].Cells[1].Value}'";
            }

            string sql = $"SELECT p.id, p.product, f.total_sum, f.paid FROM products p " +
                $"JOIN futura_info fi ON fi.id_product = p.id " +
                $"JOIN futura f ON f.id = fi. id_futura " +
                $"    WHERE {selectedNames} AND f.date >= '{dateStart}' AND f.date <= '{dateEnd}'";
            NpgsqlConnection connection = new NpgsqlConnection("Server=localhost; Port=5432; User Id=postgres; " +
                "Password=postpass; Database=KutMag");
            connection.Open();
